...
export default function CompetitorPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Fetch all data in a single coordinated load
  useEffect(() => {
    if (!competitorName) return;

    Promise.all([
      fetch('/api/competitors').then(r => r.json()) as Promise<CompetitorData[]>,
      fetch(`/api/events?competitor=${encodeURIComponent(competitorName)}&limit=200`).then(r => r.json()) as Promise<{ events: Event[] }>,
    ]).then(async ([competitors, eventsData]) => {
      const comp = competitors.find((c) => c.competitor === competitorName);
      setInfo(comp || null);
      setEvents(eventsData.events || []);

      // Load profile using competitor_id from the already-fetched events
      const competitorId = eventsData.events?.[0]?.competitor_id ?? comp?.competitor_id;
      if (competitorId) {
        try {
          const profileRes = await fetch(`/api/competitors/${competitorId}/profile`);
          if (profileRes.ok) {
            setProfile(await profileRes.json());
          }
        } catch {
          // Profile load failure is non-fatal — overview still works without it
        }
      }
